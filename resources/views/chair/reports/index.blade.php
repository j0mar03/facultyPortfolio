		...
		<div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
				...
				<div class="border-b border-gray-200 dark:border-gray-700">
					<nav class="-mb-px flex space-x-8 px-6" aria-label="Tabs">
						@foreach($managedCourses as $course)
							<a href="{{ route('reports.index', ['course_id' => $course->id, 'academic_year' => $selectedYear, 'term' => $selectedTerm]) }}"
			...
			<div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-6">
				<div class="flex items-center justify-between flex-wrap gap-4">
					<form method="GET" action="{{ route('reports.index') }}" class="flex items-center gap-4 flex-wrap">
					...
					@if($totalApproved > 0)
						<a href="{{ route('reports.download-all', ['course_id' => $selectedCourse->id, 'academic_year' => $selectedYear, 'term' => $selectedTerm]) }}"
						   class="inline-flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-md transition">							<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
