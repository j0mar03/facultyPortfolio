...
Operational indicators dependent on unavailable source tables (for example `reviews` and `audit_logs`) are reported as not measurable in the current extraction window.
